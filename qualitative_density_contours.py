"""
File: qualitative_density_contours.py | Version: 1.1.0 | Date: 2026-03-14
...
DEFAULT_SEMA_RESULTS_DIR  = os.path.join(BASE_DIR, 'results')
DEFAULT_AMFVI_RESULTS_DIR = os.path.join(BASE_DIR, 'results_AMF_VI_C')
...
N_SAMPLES       = 5000
N_GRID          = 200
MARGIN_FRAC     = 0.15   # fractional axis margin around data
...
def _kde_grid(samples: np.ndarray, x_min, x_max, y_min, y_max,
    ...
    """
    Evaluate gaussian_kde on a regular grid.
...
def plot_qualitative_contours(
    ...
    for row_idx, ds in enumerate(datasets):
        ...
        try:
            ...
            true_samples = split_data['test']
            if hasattr(true_samples, 'detach'):
                true_samples = true_samples.detach().cpu().numpy()
            else:
                true_samples = np.array(true_samples)
            # subsample if large
            if len(true_samples) > n_samples:
                idx = np.random.choice(len(true_samples), n_samples, replace=False)
...
def _parse_args():
    ...
    parser.add_argument(
        ...
        help=f'Samples per model for KDE (default: {N_SAMPLES})'
    )
    parser.add_argument(
