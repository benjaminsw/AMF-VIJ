"""
File: evaluate_threeflows_amf_vi_mmd.py | Version: 2.0.0 | Date: 2026-03-04

CHANGELOG v2.0.0:
- compute_mmd_metrics_single_iteration: replaced (flow_model) param with (generated_samples); removed internal flow_model.sample(10000)
- compute_mmd_metrics_with_iterations: added generated_samples param; threaded through to single_iteration call
- evaluate_individual_flows_mmd_with_iterations: added generated_samples param; removed per-flow internal sampling
- evaluate_mixture_model_mmd_with_iterations: added generated_samples param
- evaluate_single_sequential_dataset_mmd_with_iterations: accepts optional target_samples/generated_samples; standalone fallback preserved
- All print() warnings replaced with logging.error(); added import logging
"""
...
def compute_mmd_biased_scikit(target_samples, generated_samples, sigma='auto'):
    """Compute biased MMD with RBF kernel using scikit-learn"""
...
def compute_mmd_comparison(target_samples, generated_samples, sigma=1.0):
    ...
    mmd_squared_unbiased = compute_mmd_squared_unbiased(target_cpu, generated_cpu, kernel_func)
    mmd_unbiased = torch.sqrt(torch.clamp(mmd_squared_unbiased, min=0)).item()

    mmd_squared_biased = compute_mmd_squared_biased(target_cpu, generated_cpu, kernel_func)
...
def compute_mmd_metrics_single_iteration(target_samples, generated_samples):
    """Compute MMD metrics for both kernels — single iteration.
...
def compute_mmd_metrics_with_iterations(target_samples, generated_samples, n_iterations=10):
    ...
    Args:
        ...
        n_iterations:      Number of repetitions
    """
    ...
    for iteration in range(n_iterations):
        try:
            metrics = compute_mmd_metrics_single_iteration(target_samples, generated_samples)
            ...
            logging.error(f"Error in MMD iteration {iteration + 1}: {e}")
            continue
...
def evaluate_individual_flows_mmd_with_iterations(model, target_samples, generated_samples,
                                                   flow_names, n_iterations=10):
    ...
    with torch.no_grad():
        for i, (flow, name) in enumerate(zip(model.flows, flow_names)):
            ...
            mmd_metrics = compute_mmd_metrics_with_iterations(
                target_samples, generated_samples, n_iterations
            )
...
def evaluate_mixture_model_mmd_with_iterations(model, target_samples, generated_samples, n_iterations=10):
    ...
    return compute_mmd_metrics_with_iterations(target_samples, generated_samples, n_iterations)
...
def evaluate_single_sequential_dataset_mmd_with_iterations(dataset_name, n_iterations=10,
    ...
    get_test_data(n_samples=200_000) / flow_model.sample(). When called from
    eval_10_iters_mbatch, caller must pass both pre-computed tensors.
    """
    ...
    # ── Metrics ───────────────────────────────────────────────────────────────
    mixture_metrics = evaluate_mixture_model_mmd_with_iterations(
        model, target_samples, generated_samples, n_iterations
    )
    individual_flow_metrics = evaluate_individual_flows_mmd_with_iterations(
        model, target_samples, generated_samples, flow_names, n_iterations
    )
...
def comprehensive_mmd_evaluation_with_iterations(n_iterations=10):
    ...
    for dataset_name in datasets:
        try:
            results = evaluate_single_sequential_dataset_mmd_with_iterations(
    ...
    for dataset_name, results in all_results.items():
        ...
        mixture_row = [
            ...
            gauss_mix['mmd_unbiased_mean'], gauss_mix['mmd_unbiased_std'],
            gauss_mix['mmd_biased_mean'],   gauss_mix['mmd_biased_std'],
            gauss_mix['mmd_biased_scikit_mean'], gauss_mix['mmd_biased_scikit_std'],
            poly_mix['mmd_unbiased_mean'],  poly_mix['mmd_unbiased_std'],
            poly_mix['mmd_biased_mean'],    poly_mix['mmd_biased_std'],
            poly_mix['mmd_biased_scikit_mean'], poly_mix['mmd_biased_scikit_std'],
            'N/A', weights_status, n_iterations,
        ]
        ...
        for i, flow_name in enumerate(results['flow_names']):
            ...
            flow_row = [
                dataset_name, flow_name.upper(),
                flow_gaussian.get('mmd_unbiased_mean', 0.0),      flow_gaussian.get('mmd_unbiased_std', 0.0),
                flow_gaussian.get('mmd_biased_mean', 0.0),        flow_gaussian.get('mmd_biased_std', 0.0),
                flow_gaussian.get('mmd_biased_scikit_mean', 0.0), flow_gaussian.get('mmd_biased_scikit_std', 0.0),
                flow_polynomial.get('mmd_unbiased_mean', 0.0),    flow_polynomial.get('mmd_unbiased_std', 0.0),
                flow_polynomial.get('mmd_biased_mean', 0.0),      flow_polynomial.get('mmd_biased_std', 0.0),
                flow_polynomial.get('mmd_biased_scikit_mean', 0.0), flow_polynomial.get('mmd_biased_scikit_std', 0.0),
