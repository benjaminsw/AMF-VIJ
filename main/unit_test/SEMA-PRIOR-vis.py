...
- Plots saved to /results/convergence_plots/{dataset}_SEMA-PRIOR_*.png
...
class SequentialAMFVI(nn.Module):
    ...
    def train_mixture_weights_moving_average(self, data, epochs=500):
        ...
        print(f"🔄 Stage 2: Learning mixture weights (Moving Average | τ={self.tau} | log πk prior)...")
        
        K = len(self.flows)
        
        # Initialize MetricsTracker for convergence visualization
        metrics_tracker = MetricsTracker(method_name="SEMA-PRIOR", n_experts=K)
        ...
        for epoch in range(epochs):
            ...
            # Create responsibilities from normalized likelihoods (expand to batch dimension)
            responsibilities_np = normalized_likelihoods.detach().cpu().numpy()
...
def train_sequential_amf_vi(dataset_name='multimodal', flow_types=None, show_plots=True, save_plots=False, n_samples=100_000, tau=1.1):
    ...
    Returns:
        model: Trained SequentialAMFVI model
        flow_losses: Training losses for each flow
        weight_losses: Training losses for mixture weights
        metrics_tracker: MetricsTracker with convergence data
    """
    
    print(f"🚀 Sequential AMF-VI v2.3.0 (τ={tau} | SEMA-PRIOR) on {dataset_name}")
    print("=" * 60)
    ...
    with torch.no_grad():
        ...
        if weight_losses:
            axes[0, 2].plot(weight_losses, label=f'Weights (τ={tau}, PRIOR)', color='red', linewidth=2)
        ...
        plt.tight_layout()
        plt.suptitle(f'AMF-VI v2.3.0 (τ={tau} | SEMA-PRIOR) — {dataset_name.title()}', fontsize=16)
...
if __name__ == "__main__":
    ...
    print(f"🚀 AMF-VI v2.3.0 | flows={flow_types} | τ={TAU} | SEMA-PRIOR")
