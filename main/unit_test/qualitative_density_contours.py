"""
File: qualitative_density_contours.py | Version: 1.2.4 | Date: 2026-03-19
...
CHANGELOG:
- 1.2.4 (2026-03-19): Increase font size and rotate dataset row labels
  * Column headers: fontsize 10 → 14
  * Row labels (dataset names): fontsize 10 → 14, rotation 0 → 90
- 1.2.3 (2026-03-19): Per-dataset margin and sigma-clip overrides
  * DATASET_MARGIN_OVERRIDE: banana/bimodal_shared→0.01 (tighter), two_moons→0.08 (wider)
  * DATASET_SIGMA_OVERRIDE: BLR/BPR/Weibull→2.0σ clip (tighter than default 3.0)
  * _draw_contour: true_stats extended to 5-tuple (x_mean,x_std,y_mean,y_std,sigma)
  * sigma read from true_stats[4] if present, defaults to 3.0 for all other datasets
...
# Per-dataset margin overrides (overrides MARGIN_FRAC for specific datasets)
DATASET_MARGIN_OVERRIDE = {
    'banana':         0.01,   # tighter — compact support
    'bimodal_shared': 0.01,   # tighter — compact support
    'two_moons':      0.08,   # wider — curved manifold needs more breathing room
}

# Per-dataset sigma clipping overrides for OOD model samples
# Lower value = tighter clip (use for poorly-fitted models on real targets)
DATASET_SIGMA_OVERRIDE = {
    'BLR':     2.0,   # tighter clip — RealNVP spreads too wide
    'BPR':     2.0,
    'Weibull': 2.0,
}
...
def _draw_contour(ax, samples: np.ndarray, x_min, x_max, y_min, y_max,
    ...
    Args:
        true_stats: (x_mean, x_std, y_mean, y_std, sigma) from true_samples;
                    sigma controls clip width (default 3.0)
    """
    try:
        # Clip to mean±sigma*std of true data to exclude OOD samples
        if true_stats is not None:
            x_mean, x_std, y_mean, y_std = true_stats[:4]
            sigma = true_stats[4] if len(true_stats) > 4 else 3.0
            x_lo = x_mean - sigma * x_std
            x_hi = x_mean + sigma * x_std
            y_lo = y_mean - sigma * y_std
            y_hi = y_mean + sigma * y_std
        else:
...
def plot_qualitative_contours(
    ...
    for col_idx, header in enumerate(COL_HEADERS):
        axes[0, col_idx].set_title(header, fontsize=14, fontweight='normal', pad=4)
    ...
    for row_idx, ds in enumerate(datasets):
        ...
        row_axes[0].set_ylabel(ds_label, fontsize=14, rotation=90,
        ...
        # Shared grid bounds derived from true data (per-dataset margin override)
        margin = DATASET_MARGIN_OVERRIDE.get(ds, MARGIN_FRAC)
        x_min, x_max, y_min, y_max = _compute_grid_bounds(true_samples, margin=margin)
        sigma_clip = DATASET_SIGMA_OVERRIDE.get(ds, 3.0)
        true_stats = (
