...
class SequentialAMFVI(nn.Module):
    ...
    def __init__(self, dim=2, flow_types=None, weight_update_method='moving_average', tau=1.1):
        super().__init__()
        self.dim = dim
    ...
    def train_mixture_weights_moving_average(self, data, epochs=500):
        ...
        print(f"🔄 Stage 2: Learning mixture weights (Moving Average | τ={self.tau} | log πk prior | ε floor)...")
        ...
        for epoch in range(epochs):
            ...
            normalized_likelihoods = F.softmax((flow_log_probs_tensor + log_pi) / self.tau, dim=0)
...
def train_sequential_amf_vi(dataset_name='multimodal', flow_types=None, show_plots=True, save_plots=False, n_samples=100_000, tau=1.1):
    """Train sequential AMF-VI with learnable weights."""
    
    print(f"🚀 Sequential AMF-VI v2.3.0 (τ={tau} | SEMA-FLOOR) on {dataset_name}")
    print("=" * 60)
    ...
    model = SequentialAMFVI(dim=2, flow_types=flow_types, weight_update_method='moving_average', tau=tau)  # v2.1.0: pass tau
    ...
    with torch.no_grad():
        ...
        if weight_losses:
            axes[0, 2].plot(weight_losses, label=f'Weights (τ={tau}, FLOOR)', color='red', linewidth=2)
        ...
        plt.tight_layout()
        plt.suptitle(f'AMF-VI v2.3.0 (τ={tau} | SEMA-FLOOR) — {dataset_name.title()}', fontsize=16)
    ...
    with open(model_path, 'wb') as f:
        pickle.dump({
            ...
            'tau': tau,
            'version': '2.3.0',                  # v2.3.0: version tracking
            'abbr': 'AMFVI-SEMA-FLOOR',           # v2.3.0: abbr tracking
...
if __name__ == "__main__":
    ...
    print(f"🚀 AMF-VI v2.3.0 | flows={flow_types} | τ={TAU} | SEMA-FLOOR")
