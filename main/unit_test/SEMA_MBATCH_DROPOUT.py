"""
Version: 2.8.0
Abbr: AMFVI-SEMA-MBATCH-MDROP-ROT

CHANGELOG v2.8.0:
- Added ROTATE_DROPOUT config dict: rotation schedules for single-drop (rot1) and two-drop (rot2)
- train_flows_independently() extended: optional rotate_schedule param with per-flow stagger offset
- Per-flow stagger: flow i starts rotation at index i % len(schedule) to maximise expert diversity
- train_sequential_amf_vi() detects ROTATE_DROPOUT and builds rotation_data_list, falls back to DROP_DATASETS
- __main__ updated: added multimodal-5-rot1 and multimodal-5-rot2 dataset entries

CHANGELOG v2.7.0:
- Added DROP_DATASETS config dict for per-flow mode-dropout dataset mapping
- train_flows_independently() refactored: data→flow_data_list, dead per_expert_data stub removed
- train_sequential_amf_vi() builds per-flow drop-variant data when dataset in DROP_DATASETS
- __main__ updated: multimodal5_drop* separate models replaced by single multimodal-5 entry
...
# -----------------------------------------------------------------------
# DROP_DATASETS: per-flow mode-dropout dataset mapping (MDROP-SPEC v2.7.0)
# Keys   : dataset_name passed to train_sequential_amf_vi()
# Values : list of drop-variant dataset names, one per flow (order matters)
#          must match len(flow_types) at runtime
# Add new entries here to extend mode-dropout to other datasets.
# -----------------------------------------------------------------------
DROP_DATASETS = {
    'multimodal-5': ['multimodal5_drop0', 'multimodal5_drop1', 'multimodal5_drop2'],
}

# -----------------------------------------------------------------------
# ROTATE_DROPOUT: rotation schedules for mode-dropout training (v2.8.0)
# Keys   : dataset_name passed to train_sequential_amf_vi()
# Values : ordered list of drop-variant dataset names to rotate through.
#          All flows train on every variant sequentially (epoch budget split
#          equally). Per-flow stagger offset (i % len) maximises diversity.
# rot1   : single-mode dropout rotation (5 variants, one per mode)
# rot2   : adjacent two-mode dropout rotation (5 pairs)
# -----------------------------------------------------------------------
ROTATE_DROPOUT = {
    'multimodal-5-rot1': [
        'multimodal5_drop0',
...
class SequentialAMFVI(nn.Module):
    ...
    def train_flows_independently(self, flow_data_list, epochs=1000, lr=1e-4, rotate_schedule=None):
        """Stage 1: Train each flow independently on its assigned data.

        Args:
            flow_data_list: list of Tensors, one per flow. Use the same tensor
                            repeated (e.g. [train_data]*K) for uniform training,
                            or per-flow drop-variant tensors for MDROP-SPEC.
            epochs: training epochs for parametric flows.
            lr: learning rate for Adam optimizer.
            rotate_schedule: optional list of Tensors (one per dropout variant).
                             If provided, each flow trains sequentially on every
                             variant (epoch budget split equally), starting at a
                             per-flow stagger offset (i % len) to maximise diversity.
                             flow_data_list is ignored when rotate_schedule is set.
        """
        print("🔄 Stage 1: Training flows independently...")

        if len(flow_data_list) != len(self.flows):
            raise ValueError(
                f"flow_data_list length ({len(flow_data_list)}) must match "
                f"number of flows ({len(self.flows)})"
            )

        flow_losses = []

        for i, flow in enumerate(self.flows):
            # v2.8.0: rotation mode — stagger start by flow index for expert diversity
            if rotate_schedule is not None:
                n_variants = len(rotate_schedule)
                chunk = max(1, epochs // n_variants)
                stagger = i % n_variants
                rotation_order = [rotate_schedule[(stagger + j) % n_variants] for j in range(n_variants)]
                data = rotation_order[0]   # initial variant for shape logging
                print(f"  Training flow {i+1}/{len(self.flows)}: {flow.__class__.__name__} "
                      f"| rotate {n_variants} variants x {chunk} epochs | stagger={stagger}")
            else:
                data = flow_data_list[i]   # per-flow data (drop-variant or full)
                print(f"  Training flow {i+1}/{len(self.flows)}: {flow.__class__.__name__} "
            ...
            if len(params) == 0:
                ...
                if hasattr(flow, 'fit_to_data'):
                    # v2.8.0: for rotation, fit on last variant in rotation order (all data seen)
                    fit_data = rotation_order[-1] if rotate_schedule is not None else data
                    print(f"    Using fit_to_data() method on data shape: {fit_data.shape}")
                    flow.fit_to_data(fit_data, validate_reconstruction=False)
                    losses = []
                    with torch.no_grad():
                        for epoch in range(0, epochs, max(1, epochs // 20)):
                            try:
                                log_prob = flow.log_prob(fit_data)
            ...
            if rotate_schedule is not None:
                # v2.8.0: rotation training — iterate variants in staggered order
                for v_idx, variant_data in enumerate(rotation_order):
                    print(f"    Rotation variant {v_idx+1}/{n_variants}: data shape {variant_data.shape} | {chunk} epochs")
                    for epoch in range(chunk):
                        optimizer.zero_grad()
                        try:
                            log_prob = flow.log_prob(variant_data)
                            loss = -log_prob.mean()
                            if loss.requires_grad:
                                loss.backward()
                                optimizer.step()
                            else:
                                print(f"    Warning: Loss doesn't require grad at variant {v_idx} epoch {epoch}")
                        except RuntimeError as e:
                            if "does not require grad" in str(e):
                                print(f"    Skipping gradient step at variant {v_idx} epoch {epoch}: {e}")
                        ...
                        if epoch % 50 == 0:
                            print(f"    Variant {v_idx} Epoch {epoch}: Loss = {loss.item():.4f}")
            else:
                # Standard single-dataset training (unchanged behaviour)
...
def train_sequential_amf_vi(dataset_name='multimodal', flow_types=None, show_plots=True, save_plots=False, n_samples=100_000, tau=1.1, beta=1e-5, M=3):
    ...
    base_name = 'multimodal-5' if dataset_name in ROTATE_DROPOUT else dataset_name
    split_data = get_split_data(base_name, n_samples=n_samples)
    ...
    # Stage 1: Build per-flow data list / rotation schedule (v2.8.0)
    rotate_schedule = None
    if dataset_name in ROTATE_DROPOUT:
        # v2.8.0: rotation mode — load all variant tensors, pass as rotate_schedule
        rot_names = ROTATE_DROPOUT[dataset_name]
        rotate_schedule = [
            get_split_data(rn, n_samples=n_samples)['train'].to(device)
            for rn in rot_names
        ]
        flow_data_list = [train_data] * len(flow_types)   # unused in rotation mode
        print(f"✅ Rotation dropout ({len(rot_names)} variants, stagger per flow):")
        for j, rn in enumerate(rot_names):
            print(f"   [{j}] {rn}")
    elif dataset_name in DROP_DATASETS:
        drop_names = DROP_DATASETS[dataset_name]
        if len(drop_names) != len(flow_types):
            raise ValueError(
                f"DROP_DATASETS['{dataset_name}'] has {len(drop_names)} entries but "
                f"{len(flow_types)} flows requested. Update DROP_DATASETS to match."
            )
        flow_data_list = [
            get_split_data(dn, n_samples=n_samples)['train'].to(device)
            for dn in drop_names
        ]
        print(f"✅ Mode-dropout specialisation:")
        for ft, dn in zip(flow_types, drop_names):
            print(f"   {ft.upper():10s} → {dn}")
    else:
        flow_data_list = [train_data] * len(flow_types)
        print(f"✅ No mode-dropout: all flows train on full train_data")

    flow_losses = model.train_flows_independently(
        flow_data_list, epochs=train_epochs, lr=1e-3, rotate_schedule=rotate_schedule
    )
...
if __name__ == "__main__":
    datasets = [
        ...
        #"BPR",
        #"Weibull",
        #"multimodal-5",        # DROP_DATASETS: fixed per-flow dropout specialisation (disabled)
        "multimodal-5-rot1",   # ROTATE_DROPOUT: single-mode rotation (drop0→4, staggered)
        "multimodal-5-rot2",   # ROTATE_DROPOUT: two-mode adjacent rotation (drop01→40, staggered)
        #"Real-GMM2",
        #"Old-Faithful",
        #"Iris-3Class",
    ]
    ...
    print(f"🚀 AMF-VI v2.8.0 | flows={flow_types} | τ={TAU} | β={BETA} | M={M} | SEMA-MBATCH-MDROP-ROT")
