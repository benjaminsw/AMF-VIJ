"""
File: eval_10_iters_mbatch.py | Version: 3.0.0 | Date: 2026-03-04
...
CHANGELOG v2.0.1:
- Fixed _import_sema_mbatch() path: was looking in main/ for SEMA-MBATCH-vis.py
- Correct location is main/unit_test/SEMA_MBATCH_vis.py (subdirectory + underscores not hyphens)
- No logic changes
"""
...
def compute_metrics_over_iterations(target_samples, generated_samples, flow_model, dataset_name, n_iterations=10):
    """Compute metrics over multiple iterations and return mean/std.

    Args:
        target_samples:    Fresh samples from generate_data()
        generated_samples: Cached split_data['test']
        flow_model:        Trained flow model
        dataset_name:      Dataset identifier
        n_iterations:      Number of repetitions for mean/std estimation
    """
    all_metrics = {
        'nll': [],
        'kl_divergence': [],
    ...
    logging.info(f"    Computing metrics over {n_iterations} iterations...")
...
def evaluate_single_dataset_comprehensive(dataset_name, n_iterations=10):
    ...
    try:
        ...
        # ── Load cached test split → generated_samples ───────────────────────
        split_data = get_split_data(dataset_name)
        generated_samples = split_data['test'].to(device)
        n_eval = len(generated_samples)
        logging.info(f"  {dataset_name}: n_eval={n_eval} from cached test split (split_data['test'])")

        # ── Generate fresh target_samples at same size ────────────────────────
        target_samples = generate_data(dataset_name, n_samples=n_eval).to(device)
        logging.info(f"  {dataset_name}: target_samples generated fresh, shape={target_samples.shape}")
        ...
        mixture_metrics = compute_metrics_over_iterations(
            target_samples, generated_samples, model, dataset_name, n_iterations
        )
        ...
        for i, (flow, name) in enumerate(zip(model.flows, flow_names)):
            ...
            try:
                flow_metrics = compute_metrics_over_iterations(
                    target_samples, generated_samples, flow, dataset_name, n_iterations
                )
