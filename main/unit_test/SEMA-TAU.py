...
class SequentialAMFVI(nn.Module):
    ...
    def train_mixture_weights_moving_average(self, data, epochs=500):
        ...
        print(f"🔄 Stage 2: Learning mixture weights (Moving Average | τ={self.tau})...")
        ...
        for epoch in range(epochs):
            ...
            # v2.1.0: Temperature-scaled softmax (SEMA-TAU)
            # Before (OG): normalized_likelihoods = F.softmax(flow_log_probs_tensor, dim=0)
            # After (v2.1.0): normalized_likelihoods = F.softmax(flow_log_probs_tensor / self.tau, dim=0)
            normalized_likelihoods = F.softmax(flow_log_probs_tensor / self.tau, dim=0)
...
def train_sequential_amf_vi(dataset_name='multimodal', flow_types=None, show_plots=True, save_plots=False, n_samples=100_000, tau=1.1):
    ...
    print(f"🚀 Sequential AMF-VI v2.1.0 (τ={tau}) on {dataset_name}")
    print("=" * 60)
    ...
    with torch.no_grad():
        ...
        if weight_losses:
            axes[0, 2].plot(weight_losses, label=f'Weights (τ={tau})', color='red', linewidth=2)
        ...
        plt.tight_layout()
        plt.suptitle(f'AMF-VI v2.1.0 (τ={tau}) — {dataset_name.title()}', fontsize=16)
    ...
    with open(model_path, 'wb') as f:
        pickle.dump({
            ...
            'tau': tau,                          # v2.1.0: log tau for traceability
...
if __name__ == "__main__":
    ...
    print(f"🚀 AMF-VI v2.1.0 | flows={flow_types} | τ={TAU}")
