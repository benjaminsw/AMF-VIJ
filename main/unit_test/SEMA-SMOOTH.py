"""
Version: 2.4.0
Abbr: AMFVI-SEMA-SMOOTH

CHANGELOG v2.4.0:
- Added β uniform smoothing (beta=1e-5) on r_bar after responsibility averaging, before EMA (SEMA-SMOOTH)
- r_bar ← (1-β) * r_bar + β * (1/K) per paper Eq. (24); damps early transients
- beta stored as constructor param (default=1e-5 per paper spec)
- Builds on v2.3.0 (SEMA-FLOOR); τ, log πk prior, and ε floor retained

CHANGELOG v2.3.0:
- Added ε floor (eps=1e-5) + renorm after EMA update per paper Eqs. (26-27) (SEMA-FLOOR)
- Prevents expert collapse: no weight can reach exactly zero
- Renorm maintains simplex constraint π ∈ ΔK-1 after clamping
- Builds on v2.2.0 (SEMA-PRIOR); τ and log πk prior retained
...
class SequentialAMFVI(nn.Module):
    ...
    def __init__(self, dim=2, flow_types=None, weight_update_method='moving_average', tau=1.1, beta=1e-5):
        super().__init__()
        self.dim = dim
    ...
    def train_mixture_weights_moving_average(self, data, epochs=500):
        ...
        print(f"🔄 Stage 2: Learning mixture weights (Moving Average | τ={self.tau} | log πk prior | ε floor | β smooth)...")
        ...
        for epoch in range(epochs):
            ...
            # Get flow log probabilities on data batch
            flow_log_probs = []
            ...
            # v2.2.0: Add log πk prior term (SEMA-PRIOR) — EM-style posterior weighting per paper Eq. (22)
            # Before (v2.1.0): normalized_likelihoods = F.softmax(flow_log_probs_tensor / self.tau, dim=0)
            # After (v2.2.0): include log πk → responsibilities = softmax((log_qk + log_πk) / τ)
            log_pi = torch.log(self.weights.data.clamp(min=1e-8))
            r_bar = F.softmax((flow_log_probs_tensor + log_pi) / self.tau, dim=0)

            # v2.4.0: Uniform smoothing β (SEMA-SMOOTH) — damp early transients per paper Eq. (24)
            # r_bar ← (1-β) * r_bar + β * (1/K)  applied after averaging r_bar, before EMA
            K = len(self.flows)
            ...
            if epoch % 100 == 0:
                ...
                neff = float(np.exp(-np.sum(current_weights * np.log(current_weights + 1e-8))))
                print(f"    Epoch {epoch}: Loss = {loss.item():.4f}, Weights = {current_weights}, Neff = {neff:.3f}")
...
def train_sequential_amf_vi(dataset_name='multimodal', flow_types=None, show_plots=True, save_plots=False, n_samples=100_000, tau=1.1, beta=1e-5):
    """Train sequential AMF-VI with learnable weights."""
    
    print(f"🚀 Sequential AMF-VI v2.4.0 (τ={tau} | β={beta} | SEMA-SMOOTH) on {dataset_name}")
    print("=" * 60)
    ...
    model = SequentialAMFVI(dim=2, flow_types=flow_types, weight_update_method='moving_average', tau=tau, beta=beta)  # v2.4.0: pass beta
    ...
    with torch.no_grad():
        ...
        if weight_losses:
            axes[0, 2].plot(weight_losses, label=f'Weights (τ={tau}, β={beta})', color='red', linewidth=2)
        ...
        plt.tight_layout()
        plt.suptitle(f'AMF-VI v2.4.0 (τ={tau} | β={beta} | SEMA-SMOOTH) — {dataset_name.title()}', fontsize=16)
    ...
    with open(model_path, 'wb') as f:
        pickle.dump({
            ...
            'tau': tau,
            'beta': beta,                         # v2.4.0: log beta for traceability
            'version': '2.4.0',                   # v2.4.0: version tracking
            'abbr': 'AMFVI-SEMA-SMOOTH',          # v2.4.0: abbr tracking
...
if __name__ == "__main__":
    ...
    print(f"🚀 AMF-VI v2.4.0 | flows={flow_types} | τ={TAU} | β={BETA} | SEMA-SMOOTH")
