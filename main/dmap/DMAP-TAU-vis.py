...
- Plots saved to /results/convergence_plots/{dataset}_DMAP-TAU_*.png
...
class SequentialAMFVI(nn.Module):
    ...
    def train_mixture_weights_dirichlet_map(self, data, epochs=500, eps=1e-8):
        ...
        metrics_tracker = MetricsTracker(method_name="DMAP-TAU", n_experts=K)
        ...
        for epoch in range(epochs):
            ...
            if epoch % 100 == 0:
                ...
                logger.info(f"    Epoch {epoch}: Loss = {loss.item():.4f}, Weights = {current_weights_np}, Neff = {neff:.3f}")
...
if __name__ == "__main__":
    ...
    logger.info(f"🚀 AMF-VI v3.1.0 | flows={flow_types} | τ={TAU} | α_dir={ALPHA_DIRICHLET} | DMAP-TAU")
