"""
File: threeflows_amf_vi_weights_log_OG.py | Version: 2.2.0 | Date: 2026-03-10
Abbr: AMFVI-OG

CHANGELOG v2.2.0:
- Fixed data split: Stage 1 now trains on train_data only (was train_val_data combined)
- Fixed data split: Stage 2 now learns weights on val_data only (was train_val_data combined)
- Removed train_val_data concatenation; individual splits moved to device separately
- Added Old-Faithful and Iris-3Class to dataset list (10 → 12 datasets)
- Test data remains reserved exclusively for final evaluation and visualization
...
def train_sequential_amf_vi(dataset_name='multimodal', flow_types=None, show_plots=True, save_plots=False, n_samples=100_000):
    ...
    # Stage 1: Train flows on train_data only
    flow_losses = model.train_flows_independently(train_data, epochs=train_epochs, lr=1e-3)

    # Stage 2: Learn mixture weights on val_data only
    ...
    for dataset_name in datasets:
        ...
        try:
            model, flow_losses, weight_losses = train_sequential_amf_vi(
                ...
                show_plots=False, 
                save_plots=True,
                n_samples=100_000  # ✅ Set 1M samples here
            )
