...
def plot_responsibilities_vs_weights(tracker: MetricsTracker,
    ...
    Solid lines = r_bar (Eq.23 avg, before beta-smoothing)
    Dashed lines = pi (mixture weights after EMA)
    ...
    try:
        ...
        for k in range(tracker.n_experts):
            ...
            ax.plot(epochs, r_bar[:, k],   color=color, linestyle='-',  linewidth=1.5, label=f'resp {name}')
            ax.plot(epochs, weights[:, k], color=color, linestyle='--', linewidth=1.5, label=f'weight {name}')
...
def generate_all_plots(tracker: MetricsTracker,
    ...
    try:
        ...
        plot_weight_dynamics_neff(  # v1.3.0
            tracker,
            save_path=output_dir / f"{file_prefix}_weight_dynamics_neff.png",
